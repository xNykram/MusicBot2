...
async def test_volume_wrong_range():
    ...
    assert dpytest.verify().message().content("The volume number must be between 1 - 100")
...
async def test_volume_not_playing_songs(bot):
    ...
    pytest.MonkeyPatch.setattr(volume_module.volume.voice_client, "voice_client", voice_client)
